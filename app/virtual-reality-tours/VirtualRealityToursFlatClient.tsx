...
type VrTourFormErrors = Partial<Record<keyof VrTourFormData, string>>;

function validateField(
  name: keyof VrTourFormData,
  value: string,
): string | undefined {
  switch (name) {
    case "name":
...
export default function VirtualRealityToursFlatClient() {
  ...
  const [submitStatus, setSubmitStatus] = useState<
    "idle" | "success" | "error"
  >("idle");
  const [errors, setErrors] = useState<VrTourFormErrors>({});
  const [touched, setTouched] = useState<
    Partial<Record<keyof VrTourFormData, boolean>>
  >({});

  const inputClass = (field: keyof VrTourFormData) =>
  ...
  const handleInputChange = (
    ...
    const field = name as keyof VrTourFormData;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const nextErrors: VrTourFormErrors = {
