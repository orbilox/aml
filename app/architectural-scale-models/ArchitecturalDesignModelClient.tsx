...
type ArchModelFormErrors = Partial<Record<keyof ArchModelFormData, string>>;

function validateField(
  name: keyof ArchModelFormData,
  value: string,
): string | undefined {
  switch (name) {
    case "name":
...
export default function ArchitecturalDesignModelClient() {
  ...
  const [submitStatus, setSubmitStatus] = useState<
    "idle" | "success" | "error"
  >("idle");
  const [errors, setErrors] = useState<ArchModelFormErrors>({});
  const [touched, setTouched] = useState<
    Partial<Record<keyof ArchModelFormData, boolean>>
  >({});

  const inputClass = (field: keyof ArchModelFormData) =>
  ...
  const handleInputChange = (
    ...
    const field = name as keyof ArchModelFormData;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const nextErrors: ArchModelFormErrors = {
